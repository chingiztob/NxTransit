...
def time_dependent_dijkstra_2_transfers(graph, source, target, start_time, track_used_routes = False):
    # https://doi.org/10.1016/j.entcs.2003.12.019
    # https://bradfieldcs.com/algos/graphs/dijkstras-algorithm/
    """
    Finds the shortest path between two nodes in a graph using a time-dependent Dijkstra algorithm.
    ...
    while queue:
        ...
        for v in graph.neighbors(u):
            ...
            if v not in visited:
                ...
                ###
                # add 10 minutes to the arrival time if the route is not the same
                r_routes.add(route)
                ...
                if new_arrival_time < arrival_times[v]:
                    ...
                    if route not in r_routes:
                        new_arrival_time += 300
